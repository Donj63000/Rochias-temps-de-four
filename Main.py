...
# ================== Helpers ==================
def mix_color(hex_a: str, hex_b: str, ratio: float) -> str:
    """Interpole deux couleurs hex (#RRGGBB)."""
...
# ================== Barre Canvas « tapis » ==================
class ConveyorBar(tk.Canvas):
    """
    Barre de simulation pour un tapis :
      - jauge de vitesse (bande fine)
      - capsule représentant le produit qui progresse
      - progression dictée par la vitesse (seconds = temps de convoyage)
    Méthodes :
      - set_total(seconds, speed_hz=..., speed_ratio=...)
    ...
    def set_total(self, seconds: float, *, speed_hz: float = 0.0, speed_ratio: float = 0.0):
        self.total = max(0.0, float(seconds))
        self.elapsed = 0.0
        self.speed_hz = max(0.0, float(speed_hz))
        self.speed_ratio = max(0.0, min(1.0, float(speed_ratio)))
        self.redraw()
    ...
    def redraw(self):
        ...
        # Fond
        self.create_rectangle(0, 0, w, h, fill=CARD, outline=CARD)

        margin = 18
        track_left = margin
        track_right = max(track_left + 16, w - margin)
        track_height = 16
        track_top = (h - track_height) / 2
        track_bottom = track_top + track_height

        # Bande externe
        self.create_rectangle(track_left - 4, track_top - 6, track_right + 4, track_bottom + 6, fill=BORDER, outline=BORDER)
        self.create_rectangle(track_left, track_top, track_right, track_bottom, fill=TRACK, outline=TRACK)

        inner_width = track_right - track_left
        progress = 0.0
        if self.total > 1e-9:
            progress = max(0.0, min(1.0, self.elapsed / self.total))

        fill_x = track_left + progress * inner_width

        # Remplissage pour visualiser la distance parcourue
        if fill_x > track_left:
            fill_color = mix_color(ACCENT_DISABLED, ACCENT, max(self.speed_ratio, 0.15))
            self.create_rectangle(track_left, track_top, fill_x, track_bottom, fill=fill_color, outline="")
            self.create_line(track_left, track_top, fill_x, track_top, fill=GLOW, width=2)

        # Repères rouges fixes
        if inner_width > 0:
            x1 = track_left + inner_width / 3
            x2 = track_left + 2 * inner_width / 3
            self.create_line(x1, track_top, x1, track_bottom, fill=RED, width=2)
            self.create_line(x2, track_top, x2, track_bottom, fill=RED, width=2)

        # Capsule du produit
        if self.total > 0 or self.speed_hz > 0:
            capsule_width = 18 + 12 * self.speed_ratio
            capsule_width = min(capsule_width, inner_width)
            px = fill_x
            px = max(track_left, min(px, track_right))
            px0 = max(track_left, px - capsule_width / 2)
            px1 = min(track_right, px0 + capsule_width)
            capsule_color = mix_color(ACCENT, GLOW, min(1.0, self.speed_ratio + 0.1))
            self.create_rectangle(px0, track_top - 2, px1, track_bottom + 2, fill=capsule_color, outline="")
            self.create_line(px0, track_bottom + 2, px1, track_bottom + 2, fill=GLOW, width=2)

        # Jauge vitesse (bande fine dessous)
        gauge_top = track_bottom + 6
        gauge_bottom = gauge_top + 6
        self.create_rectangle(track_left, gauge_top, track_right, gauge_bottom, fill=BORDER, outline=BORDER)
        if inner_width > 0 and self.speed_ratio > 0:
            gx = track_left + self.speed_ratio * inner_width
            gauge_color = mix_color(ACCENT, GLOW, self.speed_ratio)
            self.create_rectangle(track_left, gauge_top, gx, gauge_bottom, fill=gauge_color, outline=gauge_color)

        # Texte de vitesse
        speed_text = f"{self.speed_hz:.2f} Hz" if self.speed_hz > 0 else "—"
        self.create_text(track_left, track_top - 10, text=speed_text, fill=SUBTEXT, anchor="w", font=("Segoe UI", 9))
...
class FourApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            pcard,
            text="Simulation produit & vitesses des 3 tapis",
            style="CardHeading.TLabel",
        ).pack(anchor="w", pady=(0, 12))
        ...
        for i in range(3):
            ...
            ttk.Label(row, text=f"Tapis {i+1}", style="Card.TLabel").grid(row=0, column=0, sticky="nw", padx=(0, 12))

            bar = ConveyorBar(row, height=58)
            bar.grid(row=0, column=1, sticky="ew", pady=(0, 4))
            txt = ttk.Label(row, text="—", style="Status.TLabel")
            txt.grid(row=0, column=2, sticky="e", padx=(12, 0))

            ttk.Label(row, text="Vitesse", style="Subtle.TLabel").grid(row=1, column=0, sticky="sw", padx=(0, 12))
            ...
            speed_bar.grid(row=1, column=1, sticky="ew", pady=(0, 2))
            speed_txt = ttk.Label(row, text="—", style="Hint.TLabel")
            speed_txt.grid(row=1, column=2, sticky="e", padx=(12, 0))

            self.bars.append(bar)
            self.bar_texts.append(txt)
            self.speed_bars.append(speed_bar)
            self.speed_texts.append(speed_txt)
    ...
    def on_reset(self):
        ...
        for pb, st in zip(self.speed_bars, self.speed_texts):
            pb["value"] = 0
            st.config(text="—")
        self.speed_values = [0.0, 0.0, 0.0]
        self.speed_scale = 100.0
    ...
    def on_calculer(self):
        ...
        self.speed_values = [f1, f2, f3]
        self.speed_scale = max(1.0, *self.speed_values)
        for i in range(3):
            tot = self.seg_totals[i]
            freq = self.speed_values[i]
            ratio = 0.0 if self.speed_scale <= 1e-9 else min(1.0, freq / self.speed_scale)
            self.bars[i].set_total(tot, speed_hz=freq, speed_ratio=ratio)
            self.bar_texts[i].config(
                text=f"{freq:.2f} Hz  •  0%  •  00:00:00 / {fmt_hms(tot)}  •  en attente"
            )
            self.speed_bars[i]["value"] = ratio * 100.0
            self.speed_texts[i].config(text=f"{freq:.2f} Hz  ({ratio*100:.0f}% de la vitesse max)")
    ...
    def on_start(self):
        ...
        if self.bar_texts:
            speed0 = self.speed_values[0] if self.speed_values else 0.0
            tot0 = self.seg_totals[0] if self.seg_totals else 0.0
            self.bar_texts[0].config(
                text=f"{speed0:.2f} Hz  •  0%  •  00:00:00 / {fmt_hms(tot0)}  •  en cours…"
            )
    ...
    def _tick(self):
        ...
        if prog >= 1.0:
            ...
            speed = self.speed_values[i] if i < len(self.speed_values) else 0.0
            self.bar_texts[i].config(
                text=f"{speed:.2f} Hz  •  100%  •  {fmt_hms(dur)} / {fmt_hms(dur)}  •  terminé"
            )
            self.seg_idx += 1
            if self.seg_idx >= 3:
                self.animating = False
            ...
            self.seg_start = now
            j = self.seg_idx
            next_speed = self.speed_values[j] if j < len(self.speed_values) else 0.0
            self.bar_texts[j].config(
                text=f"{next_speed:.2f} Hz  •  0%  •  00:00:00 / {fmt_hms(self.seg_totals[j])}  •  en cours…"
            )
        ...
        speed = self.speed_values[i] if i < len(self.speed_values) else 0.0
        self.bar_texts[i].config(
            text=f"{speed:.2f} Hz  •  {pct:5.1f}%  •  {fmt_hms(elapsed)} / {fmt_hms(dur)}  •  en cours…"
        )
